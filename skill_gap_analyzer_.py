...
if st.button("Analyze Skill Gap"):
    if uploaded_file is None:
        ...
        role_skills = job_role_profiles.get(role, [])
        ...
        st.subheader(f"Results for role: {role}")
        st.markdown(f"**Skill Match Score:** {score:.2f}%")

        st.markdown("**Matched Skills:**")
        st.write(", ".join(matched) if matched else "No matched skills found.")

        st.markdown("**Missing Skills:**")
        st.write(", ".join(missing) if missing else "No missing skills, great!")
        ...
        if missing:
            for skill in missing:
                if skill in course_recommendations:
                    ...
                    st.markdown(f"- {skill} (No course found)")
        else:
